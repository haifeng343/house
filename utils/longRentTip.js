...
const matchArea = (name,matchName)=> {
  if (name.replace(/[市区县]/ig, '') == matchName.replace(/[市区县]/ig, '')) {
    return true
  }
  return false
}
...
const matchSubway = (name, matchName)=> {
  if (name.replace(/站/ig, '') == matchName.replace(/站/ig, '')) {
    return true
  }
  return false
}
    ...
    for (var index = 0; index < length2; index++) {
      ...
      if (lianjiaData.data[index].type == "district") {
        ...
        for (var temp = 0; temp < requestData.area.length; temp++) {
          if (matchArea(requestData.area[temp].name, lianjiaData.data[index].name)) {
            isMatch = true
      ...
      if (lianjiaData.data[index].type == "resblock") {
        ...
        for (var temp = 0; temp < requestData.xiaoqu.length; temp++) {
          if (matchXiaoqu(requestData.xiaoqu[temp].name, lianjiaData.data[index].name)) {
      ...
      if (lianjiaData.data[index].type == "station") {
        ...
        for (var temp = 0; temp < requestData.subway.length; temp++) {
          if (matchSubway(requestData.subway[temp].name, lianjiaData.data[index].name)) {
            isMatch = true
...
const getIntermediaryData = (data,keywords)=> {
  return new Promise((resolve)=>{
    ...
    var results1 = null
    var results2 = null
    longrent.wiwj.rentTip({ city: data.wiwj, keywords: keywords }).then(results=>{
      results1 = results
      count++
      if(count==2) {
        resolve(returnData(results1, results2))
      }
    }).catch(()=>{
      count++
      if (count == 2) {
    ...
    longrent.lianjia.rentTip({ city: data.lj, keywords: keywords }).then(results => {
      results2 = results
      count++
      if (count == 2) {
        resolve(returnData(results1, results2))
...
function returnData2(ftxData, wbtcData) {
  ...
  if (wbtcData.result && wbtcData.result.getHouseOnMapSuggestion && wbtcData.result.getHouseOnMapSuggestion.dataList instanceof Array) {
    ...
    for (var index = 0; index < length2; index++) {
      ...
      if (data2[index].type == 2) {
        var isMatch = false
        for (var temp = 0; temp < requestData.buiness.length; temp++) {
          if (matchBusiness(requestData.buiness[temp].name, data2[index].name)) {
            isMatch = true
      ...
      if (data2[index].type == 3) {
        ...
        var isMatch = false
        for (var temp = 0; temp < requestData.xiaoqu.length; temp++) {
          var name = data2[index].name.replace(/\[.*\]/ig, '')
...
const getPersonalData = (data, keywords) => {
  ...
  return new Promise((resolve) => {
    ...
    var results1 = null
    var results2 = null
    longrent.fangtianxia.rentTip({ city: data.ftx, keywords: keywords }).then(results => {
      results1 = results
      count++
      if (count == 2) {
        resolve(returnData2(results1, results2))
      }
    }).catch(() => {
      count++
      if (count == 2) {
    ...
    longrent.wbtc.rentTip({ city: data.tc, keywords: keywords }).then(results => {
      results2 = results
      count++
      if (count == 2) {
        resolve(returnData2(results1, results2))
